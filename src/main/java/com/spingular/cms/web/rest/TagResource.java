...
public class TagResource {
     ...
     */
    @PutMapping("/tags/{id}")
    public ResponseEntity<TagDTO> updateTag(@PathVariable(value = "id", required = false) final Long id,
            @Valid @RequestBody TagDTO tagDTO) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the updated tagDTO, or with status {@code 400 (Bad Request)} if the
     *         tagDTO is not valid, or with status {@code 404 (Not Found)} if the
     *         tagDTO is not found, or with status
     *         {@code 500 (Internal Server Error)} if the tagDTO couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/tags/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<TagDTO> partialUpdateTag(@PathVariable(value = "id", required = false) final Long id,
            @NotNull @RequestBody TagDTO tagDTO) throws URISyntaxException {
