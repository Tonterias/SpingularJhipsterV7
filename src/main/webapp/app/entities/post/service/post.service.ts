...
export class PostService {
  ...
  protected convertDateFromClient(post: IPost): IPost {
    return Object.assign({}, post, {
      creationDate: post.creationDate?.isValid() ? post.creationDate.toJSON() : undefined,
      publicationDate: post.publicationDate?.isValid() ? post.publicationDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((post: IPost) => {
        ...
        post.publicationDate = post.publicationDate ? dayjs(post.publicationDate) : undefined;
      });
    }
    return res;
  }
