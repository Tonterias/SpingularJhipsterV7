...
export class CinterestComponent implements OnInit {
  ...
  trackId(index: number, item: ICinterest): number {
    return item.id!;
  }

  delete(cinterest: ICinterest): void {
    const modalRef = this.modalService.open(CinterestDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.cinterest = cinterest;
  ...
  protected onSuccess(data: ICinterest[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
