...
export class FrontpageconfigUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ frontpageconfig }) => {
      if (frontpageconfig.id === undefined) {
        ...
        frontpageconfig.creationDate = today;
      }
  ...
  protected subscribeToSaveResponse(result: Observable<HttpResponse<IFrontpageconfig>>): void {
  ...
  protected updateForm(frontpageconfig: IFrontpageconfig): void {
    this.editForm.patchValue({
      ...
      creationDate: frontpageconfig.creationDate ? frontpageconfig.creationDate.format(DATE_TIME_FORMAT) : null,
  ...
  protected createFromForm(): IFrontpageconfig {
    return {
      ...new Frontpageconfig(),
